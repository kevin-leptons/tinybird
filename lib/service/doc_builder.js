...
const marked_toc = require('marked-toc');
...
class DocBuilder {
    ...
    */
    ...
    _convert_doc(src, dest, id) {
        ...
        let toc = this._get_toc_tree(markdown)

        markdown = toc + '\n' + markdown

        let html = marked(markdown, {
            renderer: this._get_renderer(id),
            highlight: (code) => {
    ...
    _get_toc_tree(markdown) {
        return marked_toc(markdown, {
            firsth1: true
        })
    }
    ...
    _get_renderer(id) {
        ...
        renderer.link = (href, title, text) => {
            let link = url.resolve(base_url, href)
            return `
